...
if __name__ == "__main__":
    ...
    data_path = os.path.join(
        args.train, "alice_1.txt"
    )  # File path in the 'train' channel
    ...
    SEQUENCE_LENGTH = 64
    samples = [
        words[i : i + SEQUENCE_LENGTH + 1] for i in range(len(words) - SEQUENCE_LENGTH)
    ]
    ...
    model = TransformerModel(vocab_size, embed_dim=100, num_layers=2, num_heads=2).to(
        device
    )
